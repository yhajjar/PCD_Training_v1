...
- The app uses SSO; backend `/whoami` returns SSO identity plus a PocketBase user token (`pbToken`).
...
SSO users are automatically provisioned in PocketBase on first login with a default `role='user'`. This happens in backend `/whoami`:
- Email from SSO is the unique identifier
- New users get `role='user'` by default
- Existing users preserve role; identity fields (`uid`, `name`) are synced
